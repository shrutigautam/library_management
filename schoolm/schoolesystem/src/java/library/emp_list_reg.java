 ...
 */
@WebServlet(name = "emp_list_reg", urlPatterns = {"/emp_list_reg"})

public class emp_list_reg extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try
        {
          ...
          String query="select employee_id,empl_name from employee_tab where employee_id  in ( select adm_no from lib_reg )";
          PreparedStatement pst= con.prepareStatement(query);
          ResultSet rs=pst.executeQuery();
          out.print("<style>td{padding-left:50px;padding-right:50px;}th{padding-left:45px;padding-right:45px;}</style>");
            out.print("<table border='1' style='margin-left:10px;'><tr><th>Select</th><th>Employee_id</th><th>Employee Name</th></tr>");
            while(rs.next())  
        {  
            out.print("<tr>");
